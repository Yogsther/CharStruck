...
function resizeCanvas() {
    ...
    /*  SCALE =
        canvas.width < canvas.height ? canvas.width / 600 : canvas.height / 700; */
}
...
function render() {
    ...
    Alphabet.drawBlock("PLAYER", player.position.x, player.position.y, 80, 90);
...
function draw(
    ...
    if (boundByCamera) {
        x = x - camera.position.x * SCALE + canvas.width / 2 - w / 2;
        y = y - camera.position.y * SCALE + canvas.height / 2 - h / 2;
    }
