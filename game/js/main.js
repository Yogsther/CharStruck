...
document.body.addEventListener("keydown", e => {
	...
	if (showingMenu) {
		if (e.keyCode == 32) {
			...
			difficulty++
			if (difficulty > Object.keys(DIFFICULTIES).length) difficulty = 1
		} else if (e.keyCode == 40) {
			difficulty--
			if (difficulty == 0) difficulty = Object.keys(DIFFICULTIES).length
		}
	}
});
...
function render() {
	if (showingMenu) {
		...
		for (var x = -padding; x < canvas.width + padding; x += tileWidth) {
			for (
				var y = -padding; y < canvas.height + padding; y += tileHeight
			) {
				Alphabet.drawWord(
					"GROUND",
